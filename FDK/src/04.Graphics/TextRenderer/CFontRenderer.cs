	...
	protected void Initialize(string fontpath, int pt, FontStyle style) {
		try {
			if (fontpath != null && FontExists(fontpath)) {
				...
				this.textRenderer = new CSkiaSharpTextRenderer(Assembly.GetExecutingAssembly().GetManifestResourceStream(@"FOT-大江戸勘亭流 Std E.otf"), pt, style);
			}
			return;
		} catch (Exception e) {
